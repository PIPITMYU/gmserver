...
public final class StringUtils {
	...
	/**
	 * 文件上传 获得上传文件名字（毫秒+随机数）,无后缀名
	 * 
	 * @return
	 */
	public static String getFileName() {
		String fileName = null;
		String strDate = DateUtil.dateFormatString(new Date(),
				"yyyyMMddhhmmssSSS");
	...
	public static String converterToSpell(String realNames) {
		...
		try {
			...
			for (int i = 0; i < realName.length(); i++) {
				try {
					String[] tempPinyin = PinyinHelper
							.toHanyuPinyinStringArray(realName.charAt(i),
									defaultFormat);
					if (tempPinyin == null) {
